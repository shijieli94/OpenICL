...
class BaseRetriever:
    ...
    def get_labels(
        ...
        if prompt_template is not None and isinstance(prompt_template.template, Dict):
            ...
            ice_template is not None and ice_template.ice_token is not None and isinstance(ice_template.template, Dict)
        ):
    ...
    def generate_prompt(
        ...
        if prompt_template is not None and isinstance(prompt_template.template, Dict):
            ...
            ice_template is not None and isinstance(ice_template.template, Dict) and ice_template.ice_token is not None
        ):
            labels = list(ice_template.template.keys())[:]
        else:
            labels = list(set(self.test_ds[self.dataset_reader.output_column]))
    ...
    def generate_label_prompt(
        ...
        if prompt_template is not None:
            return (
                prompt_template.generate_label_prompt_item(self.test_ds[idx], ice, label, remain_sep)
                + self.prompt_eos_token
            )
        elif ice_template is not None and ice_template.ice_token is not None:
            return (
                ice_template.generate_label_prompt_item(self.test_ds[idx], ice, label, remain_sep)
                + self.prompt_eos_token
            )
        else:
            prefix_prompt = " ".join(
                list(
                    map(
                        str,
                        [self.test_ds[idx][ctx] for ctx in self.dataset_reader.input_columns],
                    )
                )
            )
