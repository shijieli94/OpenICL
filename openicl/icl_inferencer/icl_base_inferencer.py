...
from openicl.utils.api_service import (
    is_api_available,
    update_openicl_api_request_config,
)
...
class BaseInferencer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.call_api:
            self.__init_model(
                self.model_name, cache_dir, model_config, model_parallel, device_map, no_split_module_classes
            )
            self.__init_tokenizer(self.tokenizer_name, cache_dir)
        else:
    ...
    def __init_model(self, model_name, cache_dir, model_config, model_parallel, device_map, no_split_module_classes):
        ...
        if not model_parallel:
            if model_config is not None:
                self.model = self.__get_hf_model_from_config(model_name, cache_dir, model_config)
            else:
                self.model = self.__get_hf_model_from_name(model_name, cache_dir)
        else:
            if model_config is None:
                model_config = AutoConfig.from_pretrained(model_name, cache_dir=cache_dir)
            with init_empty_weights():
                empty_model = AutoModelForCausalLM.from_config(model_config, cache_dir=cache_dir)
    ...
    def __get_hf_model_from_name(self, model_name, cache_dir):
        if "t5" in model_name:
            return T5ForConditionalGeneration.from_pretrained(model_name, cache_dir=cache_dir)
        else:
            return AutoModelForCausalLM.from_pretrained(model_name, cache_dir=cache_dir)

    def __get_hf_model_from_config(self, model_name, cache_dir, model_config):
        if "t5" in model_name:
            raise TypeError("T5 model has no 'from_config' method")
        else:
            return AutoModelForCausalLM.from_config(model_config, cache_dir=cache_dir)

    def __init_tokenizer(self, tokenizer_name, cache_dir):
        if self.api_name == "opt-175b":
            self.tokenizer = GPT2Tokenizer.from_pretrained("facebook/opt-30b", use_fast=False, cache_dir=cache_dir)
        else:
...
class GenInferencerOutputHandler:
    ...
    def subprocess_write_to_json(self, output_json_filepath: str, output_json_filename: str):
        ...
        if self.accelerator is not None:
            filename = f"{output_json_filepath}/process{self.accelerator.process_index}_{output_json_filename}.json"
    ...
    def write_to_json(self, output_json_filepath: str, output_json_filename: str):
        filename = f"{output_json_filepath}/{output_json_filename}.json"
        with open(filename, "w", encoding="utf-8") as json_file:
            json.dump(self.results_dict, json_file, indent=4, ensure_ascii=False)
            json_file.close()
...
class PPLInferencerOutputHandler:
    ...
    def merge_to_main_process(self, output_json_filepath: str, output_json_filename: str):
        if self.accelerator is not None and self.accelerator.is_main_process:
            for pid in range(self.accelerator.num_processes):
                filename = f"{output_json_filepath}/process{pid}_{output_json_filename}.json"
